...
You can lookup this value by executing the powershell command ``Get-AdfsClient`` on the ADFS serverk
...
Client secret is generated by ADFS server when executing the ``Add-AdfsClient`` command with ``-GenerateClientSecret``
parameter.
