    ...
    def __init__(self):
        ...
        for setting, message in deprecated_settings.items():
            if setting in _settings:
                warnings.warn(
                    f"Setting {setting} is deprecated and it's value was ignored. {message}",
                    DeprecationWarning,
                )
        ...
        for setting in required_settings:
            if not getattr(self, setting):
                msg = f"django_auth_adfs setting '{setting}' has not been set"
                raise ImproperlyConfigured(msg)
...
class ProviderConfig:
    ...
    def _load_federation_metadata(self):
        server_url = f'https://{settings.SERVER}'
        base_url = f'{server_url}/{settings.TENANT_ID}'
        if settings.TENANT_ID == 'adfs':
    ...
    def build_authorization_endpoint(self, request, disable_sso=None, force_mfa=False):
        ...
        return f'{self.authorization_endpoint}?{query.urlencode()}'
